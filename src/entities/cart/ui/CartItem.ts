...
export const CartItemCard = ({ id, title, image, price, quantity }: CartProduct) => {
  ...
  return html`
    <div
      ...
      data-product-id="${id}"
    >
      <a href="/product/${id}" data-navigo class="shrink-0 flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-background-secondary overflow-hidden">
        <img src="${image}" alt="${title}" class="w-full h-full object-contain p-1" loading="lazy" />
      </a>

      <div class="flex flex-1 flex-col gap-1 min-w-0">
        <a href="/product/${id}" data-navigo class="text-sm font-medium text-text-primary leading-snug hover:text-primary transition-colors line-clamp-2">
          ${title}
        </a>
