...
export const ProductCard = ({ product, className = '' }: ProductCardProps) => {
  ...
  return html`
    <div data-product-card class="${cn('flex flex-col shadow-card gap-3.75 items-center w-full max-w-70 px-1.75 py-3.5 bg-background rounded-2xl overflow-hidden dark:bg-background-secondary', className)}">
      ...
      <a href="/product/${product.id}" data-navigo data-product-image class="flex justify-center items-center h-50 mb-2">
        <img src="${product.images[0]}" alt="${product.title}" class="max-h-full object-contain" loading="lazy" />
      </a>

      <div class="flex flex-col items-center gap-0.5 text-text-quinary text-sm w-full min-w-0">
        ...
        ${isInStock && product.warrantyPeriod ? html`<span class="text-xs">${product.warrantyPeriod}</span>` : ''}
      </div>
